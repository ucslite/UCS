...
function convertIdWithUnderscore(id) {
	return id.split(" ").join("_");
}

function convertIdWithSpace(id) {
	...
	if ((e.currentTarget.id).indexOf("ci_") === 0) {
		if (e.target.tagName.toLowerCase() === 'a') {
			...
			if (parent.href.indexOf('editCategory') !== -1) {
				editCategory(e.currentTarget.id.split("_").splice(2).join("_"));
			}
				...
				reOrder(e.currentTarget.id.split("_").splice(2).join("_"));
			}
...
function processSwipe(e) {
	if (e.dx < -10) {
		if (!dojo.hasClass('next_' + e.currentTarget.id, 'ci_not_visible')) {
			overlayNext(e.currentTarget.id);
		}
	}
	else if (e.dx > 10) {
		if (!dojo.hasClass('prev_' + e.currentTarget.id, 'ci_not_visible')) {
			overlayPrev(e.currentTarget.id);
		}
	}
}
		...
		if (div) {
			...
			if (dojo.query(div).closest(".productListingWidget").length > 0) {
				if (metrics[m].rank) {
					...
					overlay += "<div class='ci_overlay_triangle'>";
...
function showCategoryOverlay(metrics, event) {
	...
	for (var m in metrics) {
		var div = dojo.query('div[id^="ci_category_' + convertIdWithUnderscore(metrics[m].categoryIdentifier) + '"]')[0];	
		if (div) {
			dojo.style(div.parentNode, "position", "relative");
			...
			overlay += "<div class='ci_actions'>" +
						...
						"<a href='javascript:reOrder(\""+ metrics[m].categoryIdentifier +"\");'>" + 
	...
	dojo.query('div[id^="ci_"]').forEach(function(node, index, arr) {
		...
		if (tokens.length >= 4 && (tokens[1] === 'product' || tokens[1] === 'category' || tokens[1] === 'espot')) {
			switch (tokens[1]) {
				...
				case "category": {
					categoryHooks.push(tokens.slice(2,3) + "_" + convertIdWithSpace(tokens.slice(3).join('_')));
					break;
				}
				case "espot": {
	...
	if (data) {
		switch (data.cmd) {
			...
			case "response_metrics_category":
				showCategoryOverlay(JSON.parse(data.metrics), event);
				break;
